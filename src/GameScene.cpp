...
namespace GraphLib {
    ...
    static Animation makeStaticAnimation(sf::Texture& texture)
    {
        sf::Vector2u size = texture.getSize();

        return Animation(
            1,                                      // frameCount
            1.f,                                    // frameDuration, peu importe ici
            sf::Vector2i(size.x, size.y),           // frameSize = taille complète de l'image
            1,                                      // columns
            1,                                      // rows
            sf::Vector2i(0, 0),                     // startFrame
            0                                       // currentFrame
        );
    }
    ...
    Prey GameScene::makePrey() const
    {
        ...
        sf::Texture& texture = _game.getTexture(info.textureName);
        Animation anim = makeStaticAnimation(texture);
        ...
        _gameOver(false)
    {
        ...
        _redPos = {W / 2.f, _waterY - HOVER_OFFSET};
        sf::Texture& chickenTexture = _game.getTexture("chicken");
        Animation chickenAnim = makeStaticAnimation(chickenTexture);
